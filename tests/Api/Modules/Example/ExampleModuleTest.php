...
class ExampleModuleTest extends SharedTestCase
{
  public function testCanCreateExample(): void
  {
    ...
    $createdAt = @preg_match('/"created_at":(\d+)/', $res->getContent(), $matches) ? (int)$matches[1] : null;
    ...
    $updatedAt = @preg_match('/"updated_at":(\d+)/', $res->getContent(), $matches) ? (int)$matches[1] : null;
